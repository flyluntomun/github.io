...
function RefreshFileBox() {
  ...
  if(filesNeeded > 0) {
    if(percentage == 100 || ((filesDL + filesChecked) == filesNeeded)) {
      $("#progressInfoWS").html("ааОбаОаВаО !");
    }
  }
  $("#progressInfo").html("аЄаАаЙаЛаОаВ баКаАбаАаНаО : " + filesDL + " | аЄаАаЙаЛаОаВ аПбаОаВаЕбаЕаНаО : " + filesChecked + "<br>аббаАаЛаОбб баКаАбаАбб/аПбаОаВаЕбаИбб : " + filesRemaining);
}
...
function DownloadingFile(fileName) {
  ...
  isDownload = true;
  $("#stateLoad").html("аЁаКаАбаИаВаАаНаИаЕ " + fileName);
  $("#noBorder").attr("src", dlFile);
  $("#noBorder").attr("class", "NOPE");
  var splitSTR = fileName.split(" ");
  for (var i = 0; i<splitSTR.length; i++) {
    if(splitSTR[i] == "Workshop") {
      $("#progressInfoWS").html("аЁаКаАбаИаВаАаНаИаЕ аАаДаДаОаНаА аИаЗ аМаАббаЕббаКаОаЙ...");
    } else {
      $("#progressInfoWS").html(" - - - ");
    }
  }
